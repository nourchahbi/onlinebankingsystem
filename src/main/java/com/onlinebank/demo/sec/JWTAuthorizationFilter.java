...
public class JWTAuthorizationFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		//recuperation et verification du token
		
		String jwt = request.getHeader(SecurityParams.HEADERNAME);
		System.out.println("---------------tocken= "+jwt);
		if(jwt==null || !jwt.startsWith(SecurityParams.HEADER_PREFIX)) {
			filterChain.doFilter(request, response);
			return;
		}
		
		//decoder et recuperer le token +supprimer le prefix
